...
void processInput(GLFWwindow *window)
{
    ...
    // 在实际情况下根据处理器能力的不同
    // 可能会出现其他电脑每秒绘制更多帧
    // 也就是更高的频率调用processInput函数
    // 根据配置的不同 可能会有移动速度不同的差异
    // 因此需要确保在所有硬件上移动速度相同
    //
    // 图形程序和游戏通常会跟踪一个时间差(Deltatime)变量
    // 它储存了渲染上一帧所用的时间
    // 将所有的速度都去乘以deltaTime 
    // 结果就是如果 deltaTime很大 意味着上一帧的渲染花费了更多时间
    // 因此这一帧的速度下需要变得很高来平衡渲染所花去的时间
    // 使用这种方法 无论电脑配置如何 摄像机的速度都会相应平衡
    //
