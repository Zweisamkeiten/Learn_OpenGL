...
int main()
{
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // 首先查询uniform 变量的地址
        // 然后用 Matrix4fv后缀的glUniform函数将矩阵数据发送给着色器
        // 第一个参数是uniform的位置值
        // 第二个参数告诉OpenGL我们将要发送多少个矩阵
        // 这里是1
        // 第三个参数询问我们是否希望对我们的矩阵进行置换
        // 也就是说交换我们矩阵的行和列
        // OpenGL开发者通常使用一种内部矩阵布局
        // 叫做列主序布局
        // GLM的默认布局就是列主序
        // 所以不需要置换矩阵因此 为GL_FALSE
        // 最后一个参数是真正的矩阵数据
        // 但是GLM并不是把它们的矩阵储存为OpenGL所希望的那种
        // 因此需要使用GLM自带的函数value_ptr来变换这些数据
